...
public class DishDAO implements DAO<Dish>{
	...
	private final String QUERY_CREATE = "INSERT INTO dish (name, cal, carb, fat, prot, category, weight) VALUES (?,?,?,?,?,?,?)";
	private final String QUERY_READ = "SELECT * FROM dish WHERE id=?";
	private final String QUERY_UPDATE = "UPDATE dish SET name=?, cal=?, carb=?, fat=?, prot=?, category=?, weight=? WHERE id=?";
	...
	public boolean update(Dish dishToUpdate) {
		...
		if (!dishRead.equals(dishToUpdate)) {
			try {
				...
				if (dishToUpdate.getCategory() == null || dishToUpdate.getName().equals("") ) {
