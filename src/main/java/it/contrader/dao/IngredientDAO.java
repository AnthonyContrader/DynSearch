	...
	public boolean update(Ingredient ingredientToUpdate) {
		...
		if (!ingredientRead.equals(ingredientToUpdate)) {
			try {
				...
				if (ingredientToUpdate.getWeight() == 0.0f) {
					ingredientToUpdate.setWeight(ingredientRead.getWeight());
				}
				if (ingredientToUpdate.getCal() == 0.0f) {
					ingredientToUpdate.setCal(ingredientRead.getCal());
				}
				if (ingredientToUpdate.getCarb() == 0.0f) {
					ingredientToUpdate.setCarb(ingredientRead.getCarb());
				}
				if (ingredientToUpdate.getProt() == 0.0f) {
					ingredientToUpdate.setProt(ingredientRead.getProt());
				}
				if (ingredientToUpdate.getFat() == 0.0f) {
					ingredientToUpdate.setFat(ingredientRead.getFat());
				}
